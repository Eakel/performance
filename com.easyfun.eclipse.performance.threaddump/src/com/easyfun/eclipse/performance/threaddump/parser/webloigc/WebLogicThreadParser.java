 ...
 *
 */
public class WebLogicThreadParser extends AbstractThreadParser{
	/** 标识ThreadDump开始的字符串*/
	...
	protected void handleThreadState(String str, ThreadInfo info) {
		info.setState(WebLogicThreadState.getStateFromTitle(str));
	}
	...
	public String getThreadDesc(List<ThreadInfo> threadInfos){
		...
		for (ThreadInfo info : threadInfos) {
			...
			if(state.equals(WebLogicThreadState.STATE_UNKNOWN)){
				size1++;
			}else if(state.equals(WebLogicThreadState.STATE_OBJECT_WAIT)){
				size2++;
			}else if(state.equals(WebLogicThreadState.STATE_WAITING_ON_CONDITION)){
				size4++;
			}else if(state.equals(WebLogicThreadState.STATE_RUNNABLE)){
				size3++;
			}
		}
