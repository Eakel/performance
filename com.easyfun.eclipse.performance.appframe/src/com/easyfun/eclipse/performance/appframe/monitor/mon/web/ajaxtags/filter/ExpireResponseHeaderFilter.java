package com.easyfun.eclipse.performance.appframe.monitor.mon.web.ajaxtags.filter;
		...
		if ((this.urlPattern != null) && (this.urlPattern.length != 0)) {
			try {
				...
				for (int i = 0; i < this.urlPattern.length; i++) {
					if (this.urlPattern[i].matcher(StringUtils.substring(requestURI, CONTEXT_PATH.length())).matches()) {
